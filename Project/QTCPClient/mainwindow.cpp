...
void MainWindow::readSocket()
{
     ...
     if(header == hd.totalInfoEmployee)
     {
       ...
       qDebug() << "Data received --> "<<headerInfoEmp << '\t' << lastID <<  '\t'<< dept << '\t' << design;
       //
       ...
       if(QString(headerInfoEmp)  == hd.employeeDetail)
       {
          ...
          socketStream >> row >> column;
           ...
           ui->tableWidget->setRowCount(row.toInt());
           ui->tableWidget->setColumnCount(column.toInt());
           ...
           for(int r = 0; r < row.toInt(); ++r)
           {
                for(int col = 0; col < column.toInt(); ++col)
                {
                    ...
                    poItem->setData( Qt::DisplayRole, QString(sItem));
                     ...
                     ui->tableWidget->setItem( row.toInt(), column.toInt(), poItem );
                }
     ...
     else if(header == hd.infoDemp)
     {
        ui->searchStackedWidget->setCurrentIndex(3);
         ...
         size.clear();
         socketStream >> rec;
         qDebug() << "Count Row In " + header  << rec << '\n';
        //socketStream >> size;
       // qDebug() << "Count Row In " + header  << size << '\n';
        //socketStream >> rec;
        //qDebug() << "Count Row In " + header  << size << '\n';
         if(rec > 0)
         {
