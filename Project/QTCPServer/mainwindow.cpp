...
void MainWindow::readSocket() {
  if (dbUtils.db.isOpen()) {
    ...
    QString messageHd = QString("%1 ::<Received HEADER> -> " + header)
                            .arg(socket->socketDescriptor());
    ...
    // employee info
      ...
      QString message =
          "Received header does not much any of existing header: " + header;
...
void MainWindow::sendTotalInfoEmployee(QTcpSocket *socket, int &lastID,
  ...
  if (socket) {
    if (socket->isOpen()) {
      ...
      socketStream << hd.totalInfoEmployee.toUtf8() << quint32(lastID)
...
void MainWindow::sendInitialData(QTcpSocket *socket, QSqlQueryModel &model) {
  if (socket) {
    if (socket->isOpen()) {
      ...
      socketStream << hd.employeeDetail.toUtf8() << quint32(model.rowCount())
